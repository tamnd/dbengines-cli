...
describes what users get. Pick a format with `-o`, or let dbe choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dbe <command> -o table   # aligned columns for reading
dbe <command> -o jsonl   # one JSON object per line, for piping
dbe <command> -o json    # a single JSON array
dbe <command> -o csv     # spreadsheet friendly
dbe <command> -o tsv     # tab-separated
dbe <command> -o url     # just the URL column
dbe <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dbe <command> --fields id,title,url
```
...
```bash
dbe <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dbe <command>            # a table, because this is a terminal
dbe <command> | wc -l    # JSONL, because this is a pipe
```
