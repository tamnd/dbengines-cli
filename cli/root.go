// Package cli builds the dbe command tree on top of the dbengines library.
package cli
...
type App struct {
	client *dbengines.Client
	cfg    dbengines.Config
...
func Root() *cobra.Command {
	app := &App{cfg: dbengines.DefaultConfig()}

	root := &cobra.Command{
		Use:   "dbe",
		Short: "Browse DB-Engines database ranking",
		Long: `dbe reads the DB-Engines ranking at db-engines.com using only
the public HTML pages. It returns records as table, JSON, JSONL, CSV, TSV,
or URLs.

dbe is an independent tool and is not affiliated with DB-Engines.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = dbengines.NewClient(a.cfg)
	return nil
}
